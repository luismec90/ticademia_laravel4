...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            dataTabla = $('#table-comments').dataTable({
                "language": {
...
@section('content')
    <h1 class="section-title"><span><a class="btn btn-default btn-back" title="Ir atrás"
                                       href="{{ route('forum_path',$course->id) }}"><i class="fa fa-reply"></i></a> Foro</span>
    </h1>
    <div id="div-forum">
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="well">
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <textarea name="message" class="form-control message" placeholder="Deja un mensaje..."
                            ...
                            <td class="col-xs-3 col-sm-2 col-md-1">
                                <div class="row">
                                    <div class="col-sm-8 col-sm-offset-2">
                                        @include('layouts.partials.link_avatar_square',['user'=>$topicReply ->user])
                                    </div>
                                </div>
                                ...
                                <div class="row info-date">
                                    <div class="col-xs-12">
                                        @if($topicReply->user->isMe())
                                            <i class="delete-topic-reply fa fa-trash pull-right"
                                               data-topic-reply-id="{{ $topicReply->id }}"></i>
                                            <i class="edit-topic-reply fa fa-pencil-square-o pull-right"
                                               data-topic-reply-id="{{ $topicReply->id }}"
                                               data-message="{{ $topicReply->reply }}"></i>
                                        @endif
                                        <div class="text-muted">Publicado
                                            <b>{{ $topicReply->created_at->diffForHumans() }}</b>: {{ $topicReply->created_at }}
                                            , por <b>{{ $topicReply->user->linkFullName() }} <span
                                                        class="monitor">{{ $topicReply->user->roleName($course->id) }}</span></b>
